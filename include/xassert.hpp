...
inline void _xassert(const char* expr_str, bool expr, const char* file,
                     int line, const char* function, std::stringstream&& msg) {
  if (!expr) {
...
inline void _xassert(const char* expr_str, bool expr, const char* file,
                     int line, const char* function, std::string&& msg) {
  if (!expr) {
    std::cerr << file << ":" << line << " in " << function << "():" << std::endl
              << "Assertion failed: (" << expr_str << ")" << std::endl
              << msg << std::endl;
...
#define xassert(expr, msg) \
  _xassert(#expr, expr, __FILE__, __LINE__, __func__, msg)
#else
