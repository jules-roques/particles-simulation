...
Particle(Vector pos, Vector speed, double mass, std::string name) {
    xassert(pos.getDimension() == speed.getDimension(), "Position and speed dimensions must match.");
...
double Particle::
distanceTo(const Particle &other) const {
...
void Particle::
applyExternalForces(const std::list<ExternalForce>& extForces) {
...
void Particle::
applyInteractionForcesOn(Particle &other, const std::list<Interaction>& interactions) const {
    xassert(this != &other,
...
void Particle::
invertSpeed(size_t i) {
    xassert(i < _dimension, "i coordinate do not exist for this particle.");
    _speed[i] *= -1;
}

void Particle::
setPosCoord(size_t coord, double value) {
    xassert(coord < _dimension, "coord is too high.");
    _position[coord] = value;
}

void Particle::
addToForceCoord(size_t coord, double value) {
    xassert(coord < _dimension, "coord is too high.");
    _force[coord] += value;
}

void Particle::
addToForce(const Vector& vect) {
    xassert(vect.getDimension() == _dimension, "Vector and particle dimension must match.");
    _force += vect;
}

void Particle::
addToPosition(const Vector& vect) {
    xassert(vect.getDimension() == _dimension, "Vector and particle dimension must match.");
    _position += vect;
}

void Particle::
addToSpeed(const Vector& vect) {
    xassert(vect.getDimension() == _dimension, "Vector and particle dimension must match.");
    _speed += vect;
}
