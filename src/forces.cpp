...
void gravitationalInteraction(const Particle &source, Particle& target) {
    xassert(&source != &target, "Cannot compute force if particles given are the same.");
...
void wallsForce(Particle& target, Vector lowerBound, Vector upperBound, double epsilon, double sigma) {
    ...
    xassert(target.getPosition().isInBounds(lowerBound, upperBound), "particle must be inside the walls.");
