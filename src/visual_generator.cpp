...
void setAxRange(std::ofstream& scriptFile, std::string axName, double min, double max) {
    ...
    scriptFile << "set " << axName << "range [" << min << " : " << max << "]" << std::endl;
}

/* ---------------------------------- private ---------------------------------- */

bool VisualGenerator::
colorPaletteCanBeUsed() const {
    return _universe->getMaxForce() > 0;
}

void VisualGenerator::
writePhotoData() const {

    std::ofstream dataFile(_photoDataFileName); // Write in binary
    if (!dataFile) {
        throw std::runtime_error("Error opening file for writing: " + _photoDataFileName);
    }

    // Write binary data in the file
    for (const Particle& p : _universe->_particles) {
        const std::vector<double>& vect = p.getPosition().getData();     
        dataFile.write(reinterpret_cast<const char*>(vect.data()), vect.size() * sizeof(*vect.data()));
    }
...
void VisualGenerator::
writePhotoScript() const {

    std::ofstream scriptFile(_videoScriptName);
    if (!scriptFile) {
        throw std::runtime_error("Error opening file for writing: " + _videoScriptName);
    }

#ifdef SHOW_PROGRESS_INFOS
    scriptFile << "print \"Generating photo '" << _photoImageName << "'\"" << std::endl;
#endif
...
void VisualGenerator::
writeVideoScript(size_t numberFrames) const {
    ...
    std::stringstream ss;
    ss << "rm -rf " << _videoFolderName
       << "; mkdir " << _videoFolderName;
    if (system(ss.str().c_str()) != 0) {
        throw std::runtime_error("Error while creating folder: " + _videoFolderName);
    }

    // for the color palette if there is different forces
    if (colorPaletteCanBeUsed()) {
        scriptFile << "set cbrange [0:" << _universe->getMaxForce() << "]" << std::endl
               << "set cblab 'Force applied (Newton)' offset -2,0" << std::endl
               << "set cbtics offset -1.2,0" << std::endl
               ;//<< "set logscale cb" << std::endl;  // echelle log
    }
    ...
    scriptFile << "n = 1" << std::endl
               << "do for [i=0 : " << numberFrames * step - 1 << " : " << step << "] {" << std::endl
               << "    set output sprintf('" << _videoFolderName << "/img%03.0f.png',n)" << std::endl 
               << std::endl;
...
/* Here, we write directly in the gnuplot script a loading bar
   ...
   using the header file "progressbar.hpp".*/
    ...
    scriptFile << std::endl
               ...
               << "    progress = int(50.0 * n / " << numberFrames << ")" << std::endl
               << "    bar = \"\"" << std::endl
               << "    do for [i=1:progress] { bar = bar . \"#\" }" << std::endl
               << "    do for [i=progress+1:50] { bar = bar . \" \" }" << std::endl
               << "    shell_command = sprintf(\"echo -n \\\"\\rGenerating " << numberFrames << " images in '" << _videoFolderName << "' [%s] %i%\\\"\", bar, int(2 * progress))" << std::endl
...
void VisualGenerator::
writeVideoPlotCommand(std::ofstream& scriptFile) const {
...
void VisualGenerator::
writePhotoPlotCommand(std::ofstream& outFile) const {
    ...
    outFile << "'" << _photoDataFileName << "'"
            ...
            << " using " << dotsRange(_universe->_dimension)
            << " with points"
            << " pointtype " << _pointType
            << " pointsize " << _pointSize
            << std::endl;
...
void VisualGenerator::
writeImageSizes(std::ofstream& scriptFile) const {
    scriptFile << "set terminal pngcairo size "
               << _imageWidth << ',' << _imageHeigth << std::endl;
}

void VisualGenerator::
setAxesRanges(std::ofstream& scriptFile) const {
...
std::string VisualGenerator::
dotsRange(size_t n) const {
...
setPointSize(double pointSize) {
    xassert(pointSize > 0,
            "pointSize must be greater than 0.");
    _pointSize = pointSize;
}

void VisualGenerator::
setImageSizes(size_t width, size_t height) {
    _imageWidth = width;
    _imageHeigth = height;
}
...
generatePhoto() const {
    ...
    if (system(ss.str().c_str()) != 0) {
        throw std::runtime_error("Error while executing gnuplot script: " + _videoScriptName);
    }
}
...
void VisualGenerator::
generateVideo(size_t numberFrames) const {
    size_t nbPastStates = _universe->_nbPastStates;
    xassert(nbPastStates >= numberFrames,
            "nbFrames must be lower than the number of states the universe has been into.");

#ifdef SHOW_PROGRESS_INFOS
    std::cerr << "Data writed in '" << _universe->_pastParticlesFileName << "'" << std::endl;
#endif
    ...
    if (errorFile.is_open()) {
        ...
        if (fileSize > 0)
            std::cerr << "Warning: There were warnings while executing the gnuplot script '"
                    << _videoScriptName << "'. Check 'gnuplot_errors.txt'." << std::endl;
        else
            std::remove("gnuplot_errors.txt");
        
    } else {
        throw std::runtime_error("Error: Unable to open gnuplot error file: gnuplot_errors.txt");
    }
}